...
Switch ($command)
{
    format {
      ...
      $files = cmd /c dir /b /a-d /s src\apologiesserver tests | findstr \.py$
    ...
    mypy {
      ...
      poetry run mypy --config-file=.mypyrc src/apologiesserver tests
    }

    pylint {
      Write-Output "Running pylint checks..." 
      poetry run pylint --rcfile=.pylintrc src/apologiesserver tests
    }
}
